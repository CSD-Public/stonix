...
def regexReplace(filename, findPattern, replacePattern, outputFile="", backupname=""):
    '''
    ...
    @param replacePattern: string containing the text to replace the findPattern with
    @param outputFile: name of file to output new text to. If not supplied, output will be
                       written back to the origin file
...
def pyinstMakespec(scripts, noupx=False, strip=False, console=True, 
    ...
    @param specpath: Folder to store the generated spec file (default: current directory)
    ...
    # of macbuildlib, not necessarily the current working dir of calling script.
    # Therefore, if it is not specified, leave blank and let PyInstaller set default.
    if specpath:
        return mkspec.main(scripts, noupx=noupx, strip=strip, console=console, 
                           icon_file=icon_file, pathex=pathex, specpath=specpath)
    else:
        return mkspec.main(scripts, noupx=noupx, strip=strip, console=console, 
                           icon_file=icon_file, pathex=pathex)

def pyinstBuild(specfile, workpath, distpath, clean_build=False, noconfirm=False):
    '''
    ...
    @param clean_build: Clean PyInstaller cache and remove temporary files before building
...
def chmodR(perm, target, writemode):
    ...
    try:
        ...
        print "Error: Invalid writemode specified. Please use [a]ppend or [o]verwrite"
        raise
