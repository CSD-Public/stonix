...
from src.stonix_resources.rules.DisableAutoLogin import DisableAutoLogin


class zzzTestRuleDisableAutoLogin(RuleTest):

    def setUp(self):
        RuleTest.setUp(self)
        self.rule = DisableAutoLogin(self.config,
                                     self.environ,
                                     self.logdispatch,
                                     self.statechglogger)
