...
###############################################################################
    def __init__(self, path, tmpPath="", intent="present", configType="",
                 tagType="", logger):
        self.path = path
        ...
        # tagtype will either be "xml", "brackets", or others
        # self.configType = self.checkConfigType()
        if self.tagType == "xml":
            self.lt = "^\<"
            self.rt = "\>"
        elif self.tagType == "brackets":
            self.lt = "^\["
            self.rt = "\]"
        self.tempstring = ""
###############################################################################
    ...
    def getOpenClosedValue(self, tag, dict1):
        if self.contents:
            ...
            for line in contents:
                ...
                # elif re.search(self.lt + tag + self.rt,line.strip()):
                    foundtag = True
...
###############################################################################
    def storeContents(self, path):
        try:
            ...
            self.detailedresults = "KVTaggedConf: unable to open the " \
            "specified file"
            self.detailedresults += traceback.format_exc()
            return False
        for line in f:
